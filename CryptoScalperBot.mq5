...
//|                                              https://www.mql5.com |
//+------------------------------------------------------------------+
//| Valida si los niveles de SL y TP son válidos según el broker     |
//+------------------------------------------------------------------+
bool IsTradeLevelValid(double sl, double tp, double price)
{
    // --- Obtener el nivel mínimo de stops (Stops Level) del broker
    long stops_level = SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
    double min_stop_distance = stops_level * _Point;

    // --- Validar Stop Loss
    if(MathAbs(price - sl) < min_stop_distance)
    {
        Print("Error: El Stop Loss está muy cerca del precio. SL: ", sl, ", Precio: ", price, ", Distancia Mínima: ", min_stop_distance);
        return(false);
    }

    // --- Validar Take Profit
    if(MathAbs(price - tp) < min_stop_distance)
    {
        Print("Error: El Take Profit está muy cerca del precio. TP: ", tp, ", Precio: ", price, ", Distancia Mínima: ", min_stop_distance);
        return(false);
    }
...
input int      stop_loss_amount = 150;    // Stop Loss en pips
input int      take_profit_amount = 100;  // Take Profit en pips
...
void OnTick()
{
    ...
    // --- LÓGICA DE COMPRA (BUY SIGNAL) con cálculo de pips correcto
    if(last_low > prev_low && current_price > last_high && current_price > ema_value)
    {
        double stop_loss = NormalizeDouble(current_price - (stop_loss_amount * _Point), _Digits);
        double take_profit = NormalizeDouble(current_price + (take_profit_amount * _Point), _Digits);

        // --- Validar niveles de SL y TP antes de enviar la orden
        if(IsTradeLevelValid(stop_loss, take_profit, current_price))
        {
    ...
    // --- LÓGICA DE VENTA (SELL SIGNAL) con cálculo de pips correcto
    if(last_high < prev_high && current_price_bid < last_low && current_price_bid < ema_value)
    {
        double stop_loss = NormalizeDouble(current_price_bid + (stop_loss_amount * _Point), _Digits);
        double take_profit = NormalizeDouble(current_price_bid - (take_profit_amount * _Point), _Digits);

        // --- Validar niveles de SL y TP antes de enviar la orden
        if(IsTradeLevelValid(stop_loss, take_profit, current_price_bid))
        {
...
//+------------------------------------------------------------------+
//| Dibuja o mueve una línea horizontal en el gráfico (Eficiente)    |
//+------------------------------------------------------------------+
void DrawHorizontalLine(string name, double price, color clr, int width, ENUM_LINE_STYLE style)
{
    // --- Usar un nombre de objeto fijo para poder moverlo
    string fixed_name = StringSubstr(name, 0, StringFind(name, "_"));

    // --- Si el objeto ya existe, moverlo. Si no, crearlo.
    if(ObjectFind(0, fixed_name) >= 0)
    {
        ObjectMove(0, fixed_name, 0, 0, price);
    }
    else
    {
        if(ObjectCreate(0, fixed_name, OBJ_HLINE, 0, 0, price))
        {
            ObjectSetInteger(0, fixed_name, OBJPROP_COLOR, clr);
            ObjectSetInteger(0, fixed_name, OBJPROP_WIDTH, width);
            ObjectSetInteger(0, fixed_name, OBJPROP_STYLE, style);
            ObjectSetInteger(0, fixed_name, OBJPROP_SELECTABLE, false);
        }
    }
}
