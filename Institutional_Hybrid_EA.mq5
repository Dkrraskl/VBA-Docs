...
struct AssetProfile
{
    bool use_killzones;       // ¿Aplicar filtro de horario?
    double atr_sl_multiplier; // Multiplicador de ATR para el Stop Loss
};
...
input bool   InpUseTrailingStop = true;         // ¿Activar Trailing Stop ATR?
input double InpTrailingStopActivationR = 1.0;  // Ratio R para activar el Trailing Stop (ej. 1.0 = Breakeven)
input double InpTrailingStopAtrMultiplier = 2.5; // Distancia del Trailing Stop (múltiplo de ATR)
...
input int  InpTrendEmaPeriod = 200;             // Período de la EMA para definir la tendencia
...
AssetProfile g_profile; // Perfil de parámetros para el activo actual


//+------------------------------------------------------------------+
//| Funciones de Configuración de Perfil                             |
//+------------------------------------------------------------------+
void LoadAssetProfile()
{
    ...
    if(StringFind(symbol_name, "BTC") >= 0 || StringFind(symbol_name, "ETH") >= 0 || StringFind(symbol_name, "SOL") >= 0)
        asset_class = CRYPTO;
    else if(StringFind(symbol_name, "XAU") >= 0 || StringFind(symbol_name, "XAG") >= 0)
        asset_class = METAL;
    else if(StringLen(symbol_name) == 6) // Heurística simple para pares de Forex
        asset_class = FOREX;

    switch(asset_class)
    {
        case CRYPTO:
            Print("Perfil de Activo: CRYPTO cargado.");
            g_profile.use_killzones = false; // Mercado 24/7
            g_profile.atr_sl_multiplier = 2.5;
            break;

        case METAL:
            Print("Perfil de Activo: METAL cargado.");
            g_profile.use_killzones = true;
            g_profile.atr_sl_multiplier = 3.0; // Mayor volatilidad
            break;

        default: // FOREX y GENERIC
            Print("Perfil de Activo: FOREX/GENERIC cargado.");
            g_profile.use_killzones = true;
            g_profile.atr_sl_multiplier = 2.0;
            break;
    }
}
...
bool CBrooks_Executor::FindSignalBarAndPlaceOrder(FairValueGap &fvg, double atr)
{
    ...
    // Usamos la vela cerrada más recente (índice 1) como la vela a evaluar
    ...
    // Definición de Vela Señal: Vela de reversión forte.
    ...
    if(fvg.is_bullish && is_bullish_signal)
    {
        ...
        double take_profit = 0; // Se gestionará con Trailing Stop
...
int OnInit()
{
    //--- Cargar perfil de parámetros según el activo
    LoadAssetProfile();
    ...
    // Crear nombre de archivo de log dinámico
    string dynamic_log_name = StringFormat("%s_%s_M%d.csv",
                                          StringSubstr(InpCsvLogFileName, 0, StringLen(InpCsvLogFileName) - 4),
                                          _Symbol,
                                          _Period);

    g_logger = new CData_Logger(dynamic_log_name);
    ...
    g_handle_ema_trend = iMA(_Symbol, _Period, InpTrendEmaPeriod, 0, MODE_EMA, PRICE_CLOSE);
    if(g_handle_ema_trend == INVALID_HANDLE)
    {
        Print("Error creando el handle de la EMA de Tendencia");
        return INIT_FAILED;
    }
...
void OnTick()
{
    //--- Gestión de posiciones abiertas (se ejecuta en cada tick)
    ManageOpenPositions();

    //--- Lógica de entrada de nuevas operaciones (se ejecuta una vez por barra)
    if(IsNewBar())
    {
        //--- FILTROS GLOBALES ---
        ...
        // 3. ¿Estamos en horario operativo (Killzone)? (Solo si el perfil lo requiere)
        if(g_profile.use_killzones && !g_killzone->IsInKillzone())
        {
            g_current_state = FILTRADO;
            return;
        }

        // 4. ¿El spread es aceptable?
        ...
        if(!g_active_fvg.is_active)
        {
            ...
            {
                // Filtrar el FVG encontrado según el régimen del mercado
                if(InpUseTrendFilter)
                {
                    if(regime == REGIME_UPTREND && !g_active_fvg.is_bullish)
                    {
                        g_active_fvg.Reset(); // Ignorar FVG bajista en tendencia alcista
                        Print("Filtro de Tendencia: Ignorando FVG bajista en tendencia alcista.");
                    }
                    else if(regime == REGIME_DOWNTREND && g_active_fvg.is_bullish)
                    {
                        g_active_fvg.Reset(); // Ignorar FVG alcista en tendencia bajista
                        Print("Filtro de Tendencia: Ignorando FVG alcista en tendencia bajista.");
                    }
                }

                // Si el FVG es válido, dibujarlo
                if(g_active_fvg.is_active)
                {
...
{
    if(!InpUseTrendFilter) return REGIME_RANGE; // Si el filtro está desactivado, siempre es rango

    double ema_buffer[2];
    if(CopyBuffer(g_handle_ema_trend, 0, 1, 2, ema_buffer) < 2)
    {
        Print("Error al copiar el buffer de la EMA de tendencia.");
        return REGIME_RANGE; // Como medida de seguridad
    }

    MqlRates current_price[1];
    if(CopyRates(_Symbol, _Period, 0, 1, current_price) < 1)
    {
        Print("Error al copiar el precio actual.");
        return REGIME_RANGE;
    }

    double ema_current = ema_buffer[0];
    double ema_previous = ema_buffer[1];
    double price_close = current_price[0].close;

    if(price_close > ema_current && ema_current > ema_previous)
    {
        return REGIME_UPTREND;
    }
    else if(price_close < ema_current && ema_current < ema_previous)
    {
...
void ManageOpenPositions()
{
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        if(PositionGetSymbol(i) == _Symbol && PositionGetInteger(POSITION_MAGIC) == InpMagicNumber)
        {
            ...
            double current_sl = initial_sl;
            double current_tp = PositionGetDouble(POSITION_TP);
            long type = PositionGetInteger(POSITION_TYPE);

            double initial_risk_points = 0;
            if(type == POSITION_TYPE_BUY)
                initial_risk_points = open_price - initial_sl;
            else
                initial_risk_points = initial_sl - open_price;
            ...
            if(type == POSITION_TYPE_BUY)
                current_profit_points = current_price - open_price;
            else
                current_profit_points = open_price - current_price;

            double current_r = current_profit_points / initial_risk_points;

            if(current_r >= InpTrailingStopActivationR)
            {
                double atr = g_volatility->GetATR(0); // Usar ATR actual
                double new_sl = 0;

                if(type == POSITION_TYPE_BUY)
                {
                    new_sl = current_price - (atr * InpTrailingStopAtrMultiplier);
                    // Mover a breakeven si el R de activación es 1.0
                    if(InpTrailingStopActivationR == 1.0)
                       new_sl = fmax(new_sl, open_price);
                }
                else // Venta
                {
                    new_sl = current_price + (atr * InpTrailingStopAtrMultiplier);
                    // Mover a breakeven
                    if(InpTrailingStopActivationR == 1.0)
                        new_sl = fmin(new_sl, open_price);
                }

                // Modificar solo si el nuevo SL es mejor (más alto para compra, más bajo para venta)
