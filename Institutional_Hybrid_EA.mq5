...
#property version   "1.03" // Versión con sintaxis de punteros corregida
#property strict
...
bool CICT_Analyzer::FindSetup(FairValueGap &fvg_out)
{
    ...
    double p0 = last_extrema[0]; int pos0 = extrema_pos[0]; // Most recent extrema
    double p1 = last_extrema[1]; int pos1 = extrema_pos[1]; // Previous extrema
    double p2 = last_extrema[2]; // Extrema before previous

    // Lógica corregida para Market Structure Shift (MSS)
    // p1 es el pivote que se rompe. p0 es el nuevo extremo que confirma la ruptura.
    bool is_bullish_mss = (p2 > p1 && p0 > p2); // Se forma un Low (p1), luego un High (p2), y el nuevo High (p0) rompe el High anterior (p2)
    bool is_bearish_mss = (p2 < p1 && p0 < p2); // Se forma un High (p1), luego un Low (p2), y el nuevo Low (p0) rompe el Low anterior (p2)
...
bool CBrooks_Executor::FindSignalBarAndPlaceOrder(FairValueGap &fvg, double atr)
{
    ...
    // Restaurar el filtro de rango de la vela señal
