...
bool CData_Logger::OpenFile()
{
    ...
    if(FileSize(m_file_handle) == 0)
    {
        FileWrite(m_file_handle,
                  "Timestamp", "Symbol", "SignalType", "Direction",
                  "EntryPrice", "StopLoss", "TakeProfit", "LotSize", "Result");
    }
    return true;
}
...
void CData_Logger::LogTrade(string signal_type, string trade_direction, double entry_price,
                            double stop_loss, double take_profit, double lot_size, string result)
{
...
double CRisk_Manager::CalculateLotSize(double stop_loss_pips)
{
    ...
    if(tick_value <= 0)
    {
        Print("Valor del tick inválido. No se puede calcular el lotaje.");
        return 0.0;
    }

    // El valor del SL en la moneda de la cuenta es el número de pips por el valor de cada pip.
    // El valor de un pip se deriva del valor del tick.
    double money_per_pip = tick_value * (0.1 / _Point);
    double sl_in_money = stop_loss_pips * money_per_pip;

    if(sl_in_money <= 0) return 0.01;

    double lots = risk_amount / sl_in_money;
...
bool CBrooks_Executor::FindSignalBarAndPlaceOrder(FairValueGap &fvg, double atr)
{
    ...
    {
        double entry_price = signal_candle.high + _Point * 5;
        ...
        {
            g_logger->LogTrade("ICT+Brooks", "BUY", entry_price, stop_loss, take_profit, lot_size, "Placed");
            return true;
        }
        else
        {
            g_logger->LogTrade("ICT+Brooks", "BUY", entry_price, stop_loss, take_profit, lot_size, "Failed: " + IntegerToString(trade.ResultRetcode()));
        }
    }
    else if(!fvg.is_bullish && is_bearish_signal)
    {
        double entry_price = signal_candle.low - _Point * 5;
        ...
        {
            g_logger->LogTrade("ICT+Brooks", "SELL", entry_price, stop_loss, take_profit, lot_size, "Placed");
            return true;
        }
        else
        {
            g_logger->LogTrade("ICT+Brooks", "SELL", entry_price, stop_loss, take_profit, lot_size, "Failed: " + IntegerToString(trade.ResultRetcode()));
        }
    }
        ...
        if(OrderSelect(ticket))
        {
            if(OrderGetInteger(ORDER_MAGIC) == InpMagicNumber && OrderGetString(ORDER_SYMBOL) == _Symbol)
            {
                if(OrderGetInteger(ORDER_STATE) == ORDER_STATE_PLACED)
                {
                    ...
                    g_logger->LogTrade("Management", OrderGetString(ORDER_TYPE) == "BUY_STOP" ? "BUY" : "SELL",
                                       OrderGetDouble(ORDER_PRICE_OPEN), 0, 0, OrderGetDouble(ORDER_VOLUME_CURRENT), "Cancelled-FVG-Invalid");
                }
            }
        }
    }
