...
input double    InpTakeProfitMonetario = 100.0;     // Take Profit Fijo en USD
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(ticket > 0 && PositionSelectByTicket(ticket))
      {
            ...
            PositionGetString(POSITION_SYMBOL) == _Symbol)
         {
            ...
            // --- 1. Lógica de Take Profit Monetario ---
            if(InpTakeProfitMonetario > 0 && profit >= InpTakeProfitMonetario)
            {
               if(trade.PositionClose(ticket))
               {
                  Print("✅ Take Profit Monetario alcanzado. Posición #", ticket, " cerrada con ganancia de ", profit);
...
bool VerificarFiltrosInstitucionales(double atr)
{
   ...
   {
      if(InpModoDebug) Print("FILTRO: Fuera de Killzone.");
      return false;
   }
   ...
   if(InpUsarFiltroSpread)
   {
      ...
      if(spread > maxSpread)
      {
         ...
         if(InpModoDebug)
         {
            Print("FILTRO: Spread alto: ", DoubleToString(spread, 5),
   ...
   if(InpModoDebug) Print("--- Filtros APROBADOS ---");
   return true;
}
...
bool VerificarKillzone()
{
   ...
   if(InpModoDebug && !(enLondres || enNY))
      Print("Hora actual: ", hora, " no está en Killzone LND (", InpHoraInicioLondres, "-", InpHoraFinLondres, ") o NY (", InpHoraInicioNY, "-", InpHoraFinNY, ")");
        ...
        if(esBullish)
        {
            if(rates[i].high < rates[i-2].low) // FVG alcista
            {
                ...
                if(fvg_top - fvg_bottom >= minFVGSize)
                {
                    ...
                    totalFVGsDetectados++;
                    Print("   ✅ FVG ALCISTA VÁLIDO ENCONTRADO post-MSS. Zona: ", fvg_bottom, " - ", fvg_top);
                    return;
                }
            }
        }
        else // esBearish
        {
            if(rates[i].low > rates[i-2].high) // FVG bajista
            {
                ...
                if(fvg_top - fvg_bottom >= minFVGSize)
                {
                    ...
                    estadoActual = STATE_MONITOREANDO_FVG;
                    totalFVGsDetectados++;
                    Print("   ✅ FVG BAJISTA VÁLIDO ENCONTRADO post-MSS. Zona: ", fvg_bottom, " - ", fvg_top);
                    return;
                }
            }
        }
...
void MonitorearFVGYBuscarSenal(double atr)
{
   ...
   {
      if(InpModoDebug) Print("⏱️ FVG Timeout");
   ...
   if(!haRetornado)
   {
      // Silencioso para no llenar el log, se puede activar si se sospecha de la lógica de tolerancia
      // totalRetestsFallidos++;
      return;
   }
   ...
   {
      totalVelasRechazadas++;
      if(InpModoDebug) Print("      ❌ Vela de señal RECHAZADA");
   }
}
