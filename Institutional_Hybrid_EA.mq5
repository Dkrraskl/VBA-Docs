...
bool CData_Logger::OpenFile()
{
    ...
    if(FileSize(m_file_handle) == 0)
    {
        FileWrite(m_file_handle,
                  ...
                  "EntryPrice", "StopLoss", "TakeProfit", "LotSize", "Spread_Pips", "ATR", "Result");
    }
    return true;
}
...
void CData_Logger::LogTrade(string signal_type, string trade_direction, double entry_price,
                            ...
                            double spread_pips, double atr, string result)
{
    ...
    FileWrite(m_file_handle,
              ...
              DoubleToString(spread_pips, 1), // Pips con 1 decimal
...
bool CBrooks_Executor::FindSignalBarAndPlaceOrder(FairValueGap &fvg, double atr)
{
    ...
    if(fvg.is_bullish && is_bullish_signal)
    {
        ...
        double current_spread_pips = SymbolInfoInteger(_Symbol, SYMBOL_SPREAD) / 10.0;

        if(trade.BuyStop(lot_size, entry_price, _Symbol, stop_loss, take_profit, 0, 0, "Buy Stop by Institutional EA"))
        {
            g_logger->LogTrade("ICT+Brooks", "BUY", entry_price, stop_loss, take_profit, lot_size, current_spread_pips, atr, "Placed");
            return true;
        }
        else
        {
            g_logger->LogTrade("ICT+Brooks", "BUY", entry_price, stop_loss, take_profit, lot_size, current_spread_pips, atr, "Failed: " + IntegerToString(trade.ResultRetcode()));
        }
    }
    else if(!fvg.is_bullish && is_bearish_signal)
    {
        ...
        {
            g_logger->LogTrade("ICT+Brooks", "SELL", entry_price, stop_loss, take_profit, lot_size, current_spread_pips, atr, "Placed");
            return true;
        }
        else
        {
            g_logger->LogTrade("ICT+Brooks", "SELL", entry_price, stop_loss, take_profit, lot_size, current_spread_pips, atr, "Failed: " + IntegerToString(trade.ResultRetcode()));
        }
    }
            ...
            if(OrderGetInteger(ORDER_MAGIC) == InpMagicNumber && OrderGetString(ORDER_SYMBOL) == _Symbol)
            {
                if(OrderGetInteger(ORDER_STATE) == ORDER_STATE_PLACED)
                {
                    ...
                    g_logger->LogTrade("Management", order_type == ORDER_TYPE_BUY_STOP ? "BUY" : "SELL",
                                       ...
                                       current_spread_pips, 0, "Cancelled-FVG-Invalid");
                }
            }
        }
    }
