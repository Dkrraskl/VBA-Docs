...
#property version   "1.02" // Versión con guardia de preprocesador
#property strict
...
// Esta sección solo se compilará si el usuario lo activa.
// Esto previene errores de compilación si la librería EconomicCalendar no se encuentra.
...
// ¡IMPORTANTE! Para activar el filtro de noticias, debe establecer 'USE_CALENDAR_FILTER' en las propiedades del EA (Ctrl+I).
// Vaya a la pestaña 'Parámetros', haga clic en 'Añadir' y defina el parámetro 'USE_CALENDAR_FILTER' sin valor.
...
// ... (El resto del código de las clases permanece igual) ...
// ... (CData_Logger, CRisk_Manager, CVolatility_Filter, etc.) ...
// (Para brevedad, el código de las clases no se repite aquí,
// ya que no cambia. La única función que cambia es IsNearHighImpactNews)
...
bool CICT_Analyzer::FindSetup(FairValueGap &fvg_out)
{
    ...
    bool is_bullish_mss = false;
    bool is_bearish_mss = false;
    ...
    MqlRates candles[];
    int candles_to_copy = pos1 - pos0 + 3;
...
bool CBrooks_Executor::FindSignalBarAndPlaceOrder(FairValueGap &fvg, double atr)
{
    ...
    double avg_range = 0;
    for(int i = 1; i < 21; i++) avg_range += past_candles[i].high - past_candles[i].low;
    avg_range /= 20.0;
    ...
    {
        double entry_price = signal_candle.high + (atr * 0.05);
    ...
    {
        double entry_price = signal_candle.low - (atr * 0.05);
...
int OnInit()
{
    ...
    string log_name = StringFormat("%s_%s_M%d.csv", StringSubstr(InpCsvLogFileName, 0, StringLen(InpCsvLogFileName) - 4), _Symbol, _Period);
    ...
    if(IsNewBar())
    {
        ...
        if(g_active_fvg.is_active)
        {
            ...
            else
            {
                ...
                if((g_active_fvg.is_bullish && candle[0].close < g_active_fvg.bottom_edge) || (!g_active_fvg.is_bullish && candle[0].close > g_active_fvg.top_edge))
                {
                   g_active_fvg.Reset();
                   CancelPendingOrders();
                   g_current_state = BUSCANDO_SETUP_ICT;
...
//+------------------------------------------------------------------+
//| Lógica del Filtro de Noticias con Guardia de Preprocesador       |
//+------------------------------------------------------------------+
bool IsNearHighImpactNews()
{
#ifdef USE_CALENDAR_FILTER
    ...
    {
       Print("Error: Habilite 'Eventos del Calendario' en las opciones (Herramientas -> Opciones -> Gráficos).");
       return false;
    }
    ...
    return false; // Retorna false si el filtro no está compilado
}
